#!/bin/sh
# TODO: more comment
# TODO: make this a sourcable file with functions others call
...
if [ ! -f ./.ssh/id_rsa ]; then
   su git -c "ssh-keygen -f /var/git/.ssh/id_rsa  -t rsa -N ''"
   echo "Here is the public key of the container's 'git' user:"
   cat /var/git/.ssh/id_rsa.pub
fi
...
if [ ! -f ./.ssh/known_hosts ]; then
    if [ -n "$TRUST_HOSTS" ]; then
        ...
        su git -c "ssh-keyscan -H $TRUST_HOSTS > /var/git/.ssh/known_hosts"
    fi
fi
...
if [ ! -d ./.gitolite ] ; then
   ...
   if [ -n "$SSH_KEY" ]; then
       ...
       su git -c "cd /var/git/repositories/gitolite-admin.git && GL_LIBDIR=$(/var/git/bin/gitolite query-rc GL_LIBDIR) PATH=$PATH:/var/git/bin hooks/post-update refs/heads/master"
   fi
else
